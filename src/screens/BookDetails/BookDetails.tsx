import { Book } from 'types/Book'
import * as Styled from './BookDetails.styles'

const tempBook: Book = {
  id: 1,
  photos: ['data:image/png;base64,'],
  author: 'author',
  title: 'title',
  available: 5,
  publisher: 'publisher',
  publicationDate: new Date('03.03.1993'),
  description: 'description'
}

// interface Props {
//     book: Book
// }

const BookDetails = (): JSX.Element => (
  <Styled.DetailsContainer>
    <Styled.PictureContainer>
      <img src={tempBook.photos[0]} alt="nothing" />
      <Styled.ChangePictureButton>
        {'<'}
      </Styled.ChangePictureButton>
      <Styled.ChangePictureButton>
        {'>'}
      </Styled.ChangePictureButton>
    </Styled.PictureContainer>
    <Styled.DescriptionContainer>
      <h3>
        {tempBook.title}
      </h3>
      {tempBook.author}
      {tempBook.available}
      {tempBook.publisher}
      {tempBook.publicationDate}
      {tempBook.description}
    </Styled.DescriptionContainer>
  </Styled.DetailsContainer>
)

export { BookDetails }
